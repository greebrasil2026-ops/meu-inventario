...
URL_PLANILHA = ""
if "connections" in st.secrets and "google_script_url" in st.secrets:
...
foto_com_dados = None
# CORREÇÃO AQUI: Mudado de 'origen' para 'origem' para bater com a variável de cima
    ...
    if st.sidebar.button("💾 Enviar Direto para o Sistema"):
        if input_serie and input_modelo and input_codigo and URL_PLANILHA:
            with st.spinner("Registrando dados..."):
                ...
                try:
                    resposta = requests.post(URL_PLANILHA, data=json.dumps(dados_envio), headers={'Content-Type': 'application/json'})
...
try:
    conexao_sheets = st.connection("gsheets", type=st.connection.GSheetsConnection)
